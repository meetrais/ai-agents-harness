...
async def main() -> None:
    ...
    async with Spinner("Vercel Dataroom Analyst analyzing data"):
        result = await Runner.run(
            agent,
            "Compare FY2025 revenue, operating income, and operating cash flow with FY2024.",
            max_turns=10,
            run_config=RunConfig(
                sandbox=SandboxRunConfig(client=VercelSandboxClient())
            ),
        )

    print("\n" + "=" * 60)
    print("AGENT OUTPUT (VERCEL):")
    print("=" * 60)
