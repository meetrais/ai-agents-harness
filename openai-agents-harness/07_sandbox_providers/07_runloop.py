...
from agents.extensions.sandbox.runloop import RunloopSandboxClient
...
async def main() -> None:
    ...
    async with Spinner("Runloop Dataroom Analyst analyzing data"):
        result = await Runner.run(
            agent,
            "Compare FY2025 revenue, operating income, and operating cash flow with FY2024.",
            max_turns=10,
            run_config=RunConfig(
                sandbox=SandboxRunConfig(client=RunloopSandboxClient())
            ),
        )

    print("\n" + "=" * 60)
    print("AGENT OUTPUT (RUNLOOP):")
    print("=" * 60)
