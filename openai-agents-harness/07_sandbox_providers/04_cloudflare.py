...
from agents.extensions.sandbox.cloudflare import CloudflareSandboxClient
...
async def main() -> None:
    ...
    agent = SandboxAgent(
        name="Dataroom Analyst (Cloudflare)",
        model="gpt-5.4",
    ...
    async with Spinner("Cloudflare Dataroom Analyst analyzing data"):
        result = await Runner.run(
            agent,
            "Compare FY2025 revenue, operating income, and operating cash flow with FY2024.",
            max_turns=10,
            run_config=RunConfig(
                sandbox=SandboxRunConfig(client=CloudflareSandboxClient())
            ),
        )

    print("\n" + "=" * 60)
    print("AGENT OUTPUT (CLOUDFLARE):")
    print("=" * 60)
